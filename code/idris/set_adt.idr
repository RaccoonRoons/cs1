...
-- TA Alex at Wed. office hours approved my new implementation of foldr'
-- and my new list_rmAssist function that I've applied to list_remove
-- I have attached my list.idr file which has the edited foldr function foldr'
--used to create list_rmAssist

list_rmAssist: (eq a) => (v: a) -> (acc: list a) -> (h: a) -> list a
list_rmAssist v acc h = ite (eql v h)
                          acc
                          (h :: acc)

list_remove: (eq a) => (v: a) -> (l: list a) -> list a
list_remove v l = foldr' (list_rmAssist v) l nil
...
set_intersection (mkSet (h::t)) (mkSet l2) = ite (member h (l2))
                                              (mkSet (h :: (intersect t l2)))
                                              (mkSet (intersect t l2))
...
set_forall p (mkSet l) = eql (filter p l) l
...
-- given a list of a's and a list of b's, return a litst of all a-b pairs

--pairelt takes an elt h and a list b and returns a list of all pairs,
--(h, e), where e is an element of b
pairelt: a -> list b -> list (pair a b)
pairelt someA nil = nil
pairelt someA (hb::tb) = (mkPair someA hb)::(pairelt someA tb)
...
-- cardinality of the powerset of a set of cardinality n is 2^n.
-- set_powerset: (s: set a) -> set (set a)
-- set_powerset mkSet nil = mkSet ([]) <--fix this later
